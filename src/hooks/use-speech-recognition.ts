...
export function useSpeechRecognition(onFinalTranscript: (transcript: string) => void) {
  ...
  const onEnd = useCallback(() => {
    if (isComponentMounted.current && recognitionRef.current && recognitionRef.current.continuous) {
  ...
  return { isListening, interimTranscript, startListening, stopListening, setLanguage };
}
